...
class ComputeLoss:
    ...
    def __call__(
        ...
        #  x, cls_score_list, reg_distri_list 输入head层的三个特征图，分类分数预测，回归预测
    ...
    def preprocess(self, targets, batch_size, scale_tensor):
        """
       每个batch的目标框的个数扩充到batch中最大的目标框的个数，并且转化到原图大小
        """
...
class VarifocalLoss(nn.Module):
    ...
    def forward(self, pred_score, gt_score, label, alpha=0.75, gamma=2.0):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            temp = F.binary_cross_entropy(pred_score, gt_score.to(pred_score), reduction="none")
            temp = temp * weight
            temp = temp.sum()
